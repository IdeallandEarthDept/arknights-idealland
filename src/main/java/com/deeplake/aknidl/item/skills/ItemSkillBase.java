package com.deeplake.aknidl.item.skills;

import com.deeplake.aknidl.IdlFramework;
import com.deeplake.aknidl.init.ModCreativeTab;
import com.deeplake.aknidl.item.ItemBase;
import com.deeplake.aknidl.util.CommonFunctions;
import com.deeplake.aknidl.util.IDLSkillNBT;
import com.deeplake.aknidl.util.MessageDef;
...
import static com.deeplake.aknidl.util.CommonDef.G_SKY;
import static com.deeplake.aknidl.util.CommonDef.TICK_PER_SECOND;
import static com.deeplake.aknidl.util.CommonFunctions.SendMsgToPlayer;
import static com.deeplake.aknidl.util.CommonFunctions.isShiftPressed;
import static com.deeplake.aknidl.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    public ItemSkillBase(String name) {
        ...
        setMaxStackSize(1);
        setNoRepair();
        setCreativeTab(ModCreativeTab.AKN_SKILL);
    }
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "aknidl.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("aknidl.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("aknidl.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("aknidl.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("aknidl.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("aknidl.skill.shared.lv_max") : "";

        return I18n.format("aknidl.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("aknidl.skill.shared.duration_desc", val);
    }
